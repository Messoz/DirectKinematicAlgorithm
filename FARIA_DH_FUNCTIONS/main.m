    function [ Ar, An, dh_meas ] = main(num_joints, home_joint_pos, joint_type_flag, ee_disp, test_joints, plot )
...
%     str = strcat(str,'.xlsx');
%     fstr = strcat(folder,'/');
%     T{i} = xlsread(strcat(fstr,str));
%     T{i} = T{i}(:,10:12).*0.001; %trasformo in metri
% 
% end

% Terzo Dataset
% for i=1:num_joints
%     str = strcat('M', num2str(i));
%     str = strcat(str,'.xlsx');
%     fstr = strcat(folder,'/');
%     T{i} = xlsread(strcat(fstr,str));
%     T{i} = T{i}(:,10:12).*0.001; %trasformo in metri 
...
% for i=1:num_joints
%     str = strcat('M', num2str(i));
%     str = strcat(str,'scara.xlsx');
%     fstr = strcat(folder,'/');
%     T{i} = xlsread(strcat(fstr,str));
%     T{i} = T{i}(:,7:9).*0.001; %trasformo in metri 10 12
% end
% 
% for i=1:num_joints
%     str = strcat('J', num2str(i));
...
%     fstr = strcat(folder,'/');
%     T{i} = xlsread(strcat(fstr,str));
%     T{i} = T{i}(:,10:12).*0.001; %trasformo in metri 10 12
% end
% 
% for i=1:num_joints
%     str = strcat('J', num2str(i));
...
% end
% for i=1:num_joints
%     str = 'IRB14000DATA.xlsx';
%     fstr = strcat(folder,'/');
%     T{i} = xlsread(strcat(fstr,str),i);
%     T{i} = T{i}(:,2:4).*0.001; %trasformo in metri 10 12
% end
% 
% for i=1:num_joints
%     str = strcat('J', num2str(i));
...
% end
for i=1:num_joints
    str = 'TCP_FANUCPROVALAB_2.xls';
    fstr = strcat(folder,'/');
    T{i} = xlsread(strcat(fstr,str),i);
    T{i} = T{i}(:,1:3).*0.001; %trasformo in metri 10 12
end
